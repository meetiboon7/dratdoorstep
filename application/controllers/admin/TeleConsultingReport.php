...
class TeleConsultingReport extends GeneralController {
	...
	{	
		...
		$this->load->view('admin/TeleConsultingReport', $data);
		$this->load->view(FOOTER);

		
	}
	public function generateTeleCosultingReport(){
		...
		$this->excel->getActiveSheet()->setCellValue('E1', 'Type of Teleconsulting');
		$this->excel->getActiveSheet()->setCellValue('F1', 'Total');
		$this->excel->getActiveSheet()->setCellValue('G1', 'Discount');
		$this->excel->getActiveSheet()->setCellValue('H1', 'Voucher');
		$this->excel->getActiveSheet()->setCellValue('I1', 'Credit');
		
      
               
         $this->db->select('appointment_book.txndate,member.name,appointment_book.service,appointment_book.city_name,appointment_book.type_of_teleconsulting,appointment_book.total,appointment_book.discount,appointment_book.voucher,appointment_book.credit');
		 ...
		 $this->db->where('appointment_book.type_of_teleconsulting !=',NULL);
