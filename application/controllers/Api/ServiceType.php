...
class ServiceType extends REST_Controller
{
     ...
     * )
     */
    public function service_type_api_ci_get()
    {
        ...
        if (isset($auth)) {
            ...
            if(count($bearer))
            {
                try
                {
                    ...
                    $service_type_details = $this->db->get()->result_array();

                    if (count($service_type_details) > 0) {

                     

                        $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Service Type Listing";
                        $apiResponse->data['nurse_service_type'] = $service_type_details;
                    ...
                    else
                    {
                        ...
                        $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Service Type Not Found";
                        $this->response($apiResponse, REST_Controller::HTTP_OK);
                    }
