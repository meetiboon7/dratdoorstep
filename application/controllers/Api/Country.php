...
class Country extends REST_Controller
{
     ...
     * )
     */
    public function country_api_ci_get()
    {
        ...
        if (isset($auth)) {
            ...
            if(count($bearer))
            {
                try
                {
                    ...
                    $country_Details = $this->db->get()->result_array();
                    ...
                    if (count($country_Details) > 0) {
                        ...
                        $apiResponse->message = "Country Listing";
                        $apiResponse->data['country'] = $country_Details;
                    ...
                    else
                    {
                        //throw new Exception("Invalid Token Data", 1);
                         $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Country Not Found";
                        $this->response($apiResponse, REST_Controller::HTTP_OK);
                    }
