...
class LabTestType extends REST_Controller
{
     ...
     * )
     */
    public function lab_test_type_api_ci_get()
    {
        ...
        if (isset($auth)) {
            ...
            if(count($bearer))
            {
                try
                {
                    ...
                    $lab_test_type_details = $this->db->get()->result_array();

                    if (count($lab_test_type_details) > 0) {

                     

                        $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Lab Test Type Listing";
                        $apiResponse->data['lab_test_type'] = $lab_test_type_details;
                    ...
                    else
                    {
                        //throw new Exception("Invalid Token Data", 1);
                         $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Lab Test Type Not Found";
                        $this->response($apiResponse, REST_Controller::HTTP_OK);
                    }
