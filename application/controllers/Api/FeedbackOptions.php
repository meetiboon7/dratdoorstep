...
class FeedbackOptions extends REST_Controller
{
    ...
    /**
     * @OA\Get(
     *     path="/feedback_option",
     *     tags={"Feedback Option"},
     ...
     * )
     */
    public function feedbackoption_api_ci_get()
    {
        ...
        if (isset($auth)) {
            ...
            if(count($bearer))
            {
                try
                {
                    ...
                    $this->db->select('*');
                    $this->db->from('feedback_options');
                    $feedback_options_Details = $this->db->get()->result_array();

                    if (count($feedback_options_Details) > 0) {

                      //$specific[]=$city_Details;

                        $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Feedback Option Listing";
                        $apiResponse->data['feedback_options'] = $feedback_options_Details;
                    ...
                    else
                    {
                         $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Feedback Option Not Listing";
