...
class PrivacyPolicy extends REST_Controller
{
     ...
     *     path="/privacy_policy",
     *     tags={"Privacy & Policy"},
     *     operationId="getPrivacyAndPolicy",
     ...
     * )
     */
    public function privacy_policy_api_ci_get()
    {
        ...
        if (isset($auth)) {
            ...
            if(count($bearer))
            {
                try
                {
                    ...
                    $this->db->select('*');
                    $this->db->from('text');
                     $this->db->where('text_id',2);
                    $privacy_Details = $this->db->get()->result_array();
                    // echo "<pre>";
                    // print_r($terms_Details);

                    if (count($privacy_Details) > 0) {



                        $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Privacy and Policy Details Received Successfully";
                        $apiResponse->data['html'] =  $privacy_Details[0][html];
                    ...
                    else
                    {
                        $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "Privacy and Policy Details Not Received";
