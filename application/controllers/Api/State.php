...
class State extends REST_Controller
{
     ...
     * )
     */
    public function state_api_ci_get()
    {
        ...
        if (isset($auth)) {
            ...
            if(count($bearer))
            {
                try
                {
                    ...
                    $this->db->select('*');
                    $this->db->from('state');
                    $state_Details = $this->db->get()->result_array();
                    ...
                    if (count($state_Details) > 0) {
                        ...
                        $apiResponse->message = "State Listing";
                        $apiResponse->data['state'] = $state_Details;
                    ...
                    else
                    {
                        //throw new Exception("Invalid Token Data", 1);
                         $apiResponse = new Api_Response();
                        $apiResponse->status = true;
                        $apiResponse->message = "State Not Found";
                        $this->response($apiResponse, REST_Controller::HTTP_OK);
                    }
