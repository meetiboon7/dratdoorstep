                         ...
                         <div class="form-group col-md-6 mr-7" >
								...
								<div class="row">
									<div class="form-group col-md-6 daily_days" style="display:none;">
										<label>Days *</label>
										<input type="text" class="form-control" placeholder="Days" name="days" id="days" onkeypress="return isNumberKey(event)"/>
...
//alert(doctor_type_validate);
	if(member!="none" &&  address!="none" && date!="" && time!="" && type_validate!="none")
	{
			jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url('addNurseCart'); ?>",
			dataType: 'html',
			data: {member:member,date: date, time: time,type_validate:type_validate,address:address,complain:complain,days:days},
